...
function App() {
  ...
  useEffect(() => {
    ...
    const unlistenCompleted = listen<number>("indexing-completed", (event) => {
      ...
      if (indexingStartTime) {
        const duration = Date.now() - indexingStartTime;
        setIndexingDuration(duration);
  ...
  const formatIndexingDuration = (milliseconds: number) => {
  ...
  return (
    <div className="h-screen bg-zinc-950 text-zinc-300 border-5 rounded-xl border-zinc-900/70 flex flex-col">
      ...
      {indexingDuration && (
        <div className="fixed bottom-4 right-4 bg-zinc-900/90 backdrop-blur-sm border border-zinc-800 rounded-lg px-3 py-2 flex items-center gap-2 shadow-lg">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs font-mono text-zinc-300">
              {formatIndexingDuration(indexingDuration)}
            </span>
          </div>
        </div>
      )}
